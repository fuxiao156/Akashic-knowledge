...
import { isArticleResource, isFileResource, isMediaResource } from "@/utils/resource.guard";

const FileItem = (props: { resource: IResource<TFile> }) => {
    const { resource } = props;
    return (
            <a href={resource.data.url} download="">download</a>
    );
};
...
const MediaItem = (props: { resource: IResource<TMedia> }) => {
    const { resource } = props;
    return (
            <a href={resource.data.src} download="">download</a>
    );
};
...
const ResourceSwitch = (props: { resource: IResource<TFile | TMedia | TArticle> }) => {
    const { resource } = props;
...
const ResourceTypeLabel = (props: { type: IResource<TFile | TMedia | TArticle>['type']}) => {
    const { type } = props;
    ...
    switch (type) {
        ...
        case 'media':
            return <span className="bg-green-500 text-white p-1 rounded">多媒体</span>;
        case 'article':
...
export const ResourceListItem = (props: { resource: IResource<TFile | TMedia | TArticle> }) => {
    const { resource } = props;
    return (
        <div className="p-5 rounded-sm">
            <div className="whitespace-nowrap">
                <h1 className="text-2xl font-bold m-5 inline-block">{resource.title}</h1>
